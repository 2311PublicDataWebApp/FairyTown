							...
							<c:forEach items="${rList }" var="ride" varStatus="i">
								<tr class="ride_list_tr">
									...
									<td class="list_third_td">
										<a href="/admin/ridedetail.ft?rideNo=${ride.rideNo }">${ride.rideDate}</a>
									</td>
									<td class="list_fourth_td">
										<a href="/admin/ridedetail.ft?rideNo=${ride.rideNo }">${ride.rideReason}</a>
									</td>
							...
							<tr align="center" class="pgn">
								<td colspan="5">
									<c:if test="${pInfo.startNavi ne '1' }">
										<a href="/admin/ridelist.ft?page=${pInfo.startNavi - 1 }">[이전]</a>
									</c:if> 
									<c:forEach begin="${pInfo.startNavi }" end="${pInfo.endNavi }" var="p">
										<a href="/admin/ridelist.ft?page=${p }">${p }</a>
									</c:forEach> 
										<c:if test="${pInfo.endNavi ne pInfo.naviTotalCount }">
											<a href="/admin/ridelist.ft?page=${pInfo.endNavi + 1 }">[다음]</a>
										</c:if>
								</td>
							</tr>
						</c:when>
